...
Thus, the MLP must have a ReLU activation function between each hidden layer, giving the final model structure as laid out in Figure~\ref{fig:model_arch}. 
...
The complete dataset is made up of current density samples \emph{\textbf{(ask Alex if this is the correct term)}} from 7 cyclic voltammetry experiments -- see Table~\ref{tab:dataset}. These were collected via potentiostat with a scanrate of \SI{100}{\milli\volt\per\second}. Each experiment is made up of 264 rows, with columns for current (I / \unit{\ampere}), voltage step (E / \unit{\volt}), active electrode mass (M / \unit{\milli\gram}), temperature (T / \unit{\degreeCelsius}), electrolysis voltage (V$_e$ / \unit{\volt}), electrolysis duration (D / \unit{\hour}), and scan direction (S). The last feature tells the model if the particular point is on the charge or discharge curve and is represented by either +1 (charging)  or -1 (discharging).  

\begin{table}
\caption{Characteristics of each experiment that constitutes the dataset used for training our model. The scanrate was \SI{100}{\milli\volt\per\second} in all cases.}
\label{tab:dataset}
\small
\centering
\begin{tblr}{
...
Experiment & M / mg  & T / $^{\circ}$& V$_e$ / V & D / h & Electrolyte \\
...
\begin{figure*}
 ...
 \caption{A plot reporting the average normalised $C_{sp}$ over 4 runs per experiment per experimental setting. Error bars indicate 95\% confidence intervals over runs.}
 \label{fig:results}
\end{figure*}
...
\end{equation}
where $M$ is the number of experiments, $a_j$ is the actual area between the curves for a given experiment, and $\hat{a}_j$ is the area calculated using predicted current values.  The losses are combined for final  objective for the MLP:
\begin{equation}
  \label{eq:loss}
...
\begin{table}
\caption{Average error \% of each experiment over 4 experimental runs.}
\label{tab:errors}
\small
\centering
\begin{tblr}{
...
Experiment      & Base    & {Base\\+D}   & {Base\\+ D\\+S} & {Base\\+D\\+S\\+A} \\
BCMS2           & 11.491            & 6.304                        & 4.745                              & 0.733                              \\
BCMS2.1         & 28.385            & 25.788                       & 17.547                             & 62.284                             \\
BCMS3           & 0.552             & 10.432                       & 2.449                              & 19.004                             \\
BCMS6           & 6.482             & 7.548                        & 8.757                              & 8.310                              \\
BCMS7           & 8.795             & 2.582                        & 10.220                             & 9.344                              \\
BCMS8           & 4.860             & 4.703                        & 3.490                              & 5.534                              \\
BCMS9           & 48.238            & 24.038                       & 3.645                              & 24.173                             \\
Avg.            & 15.543            & 11.628                       & \textbf{7.265}                     & 18.483                             
\end{tblr}
\end{table}
\textbf{\emph{THIS SECTION IS INCOMPLETE, DO NOT SUBMIT AS-IS}}
